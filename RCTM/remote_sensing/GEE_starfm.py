...
def export_landsat_collection(collection, roi_asset_path, bucket_name, directory_path, storage_client, overwrite=True):
  ...
  for i in range(0, count):
    ...
    if available_slots>0:
      ...
      submitted_dates.append(dates[i])
      filepaths.append('{}{}.tif'.format(directory_path, export_names[i]))
      available_slots-=1
  
  return submitted_dates, task_ids, filepaths

def export_modis_collection(modis_collection, roi_asset_path, bucket_name, out_directory_path, storage_client, landsat_dates=[], date_download_interval = 5, landsat_dir=None, overwrite=False):
  ...
  landsat_dates = landsat_dates.to_list()
  #landsat_dates = landsat_dates.dt.strftime('%Y_%m_%d').to_list()
  ...
  for i in range(0, count):
    ...
    if names[i] in landsat_dates or i%date_download_interval==0:
      ...
      if available_slots>0:
        ...
        filepaths.append('{}MCD43A4_{}.tif'.format(out_directory_path, names[i]))
        available_slots-=1
