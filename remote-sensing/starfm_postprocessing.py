...
def get_ndvi_nc(starfm_in_dir, out_dir, out_name, bucket_name):
  ...
  bucket = storage_client.get_bucket(bucket_name)
  starfm_df = psi.get_sfm_date_df(starfm_in_dir, bucket).reset_index(drop=True)
...
def get_spatial_RCTM_params(NLCD_in_dir, RAP_in_dir, param_file, param_out_name, bucket_name, param_type='starfm'):
  ...
  utils.gs_write_blob(os.path.join(path_to_temp, 'temp_spatial_params_write.tif'), param_out_name, bucket)
...
def daymet_qaqc(ds, bucket_name=None):
...
def gen_covariates(starfm_in_dir, daymet_in_dir,  out_dir, out_name, bucket_name, start_date, end_date, gap_fill = True):
  ...
  if gap_fill==True:
    covariate_DataArray = daymet_qaqc(covariate_DataArray)
...
def image_average_variables(ds, variable_list, plot_dir=None):
