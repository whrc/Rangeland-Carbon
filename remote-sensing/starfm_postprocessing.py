...
def gen_covariates(starfm_in_dir, daymet_in_dir, out_dir, out_name, bucket_name):
  ...
  for index, row in starfm_df.iterrows():
    ...
    if len(daymet_df.loc[daymet_df['im_date']==row['im_date']])>0:
      ...
      ndvi=(starfm.values[2]-starfm.values[1])/(starfm.values[2]+starfm.values[1])
      ndvi[np.isnan(ndvi)] = 0
      
      evi = 2.5 * ((starfm.values[2]-starfm.values[1])/(starfm.values[2] + 6*starfm.values[1] - 7.5*starfm.values[0] + 1))
      evi[np.isnan(evi)] = 0
  ...
  covariate_DataArray.rio.transform=starfm.rio.transform
...
def daymet_qaqc(covariate_nc, bucket_name):
  ...
  ds['rolling_std_evi'] = ds['evi'].rolling(time=365, center=True, min_periods=1).std()
  
  ds['rolling_median_ndvi'] = ds['ndvi'].rolling(time=14, center=True, min_periods=1).mean()
  ds['rolling_median_evi'] = ds['evi'].rolling(time=14, center=True, min_periods=1).mean()
  
  ds['qa'] = xr.where((ds['ndvi'] < 0) & (ds['evi'] < 0) | (ds['daymet'] < 0) | (abs(ds['ndvi'] - ds['rolling_median_ndvi']) > ds['rolling_median_ndvi']+ds['rolling_std_ndvi']*2) | (abs(ds['evi'] - ds['rolling_median_evi']) > ds['rolling_median_evi']+ds['rolling_std_evi']*2), 1, 0)
  
  ds['ndvi'] = ds['ndvi'].where((ds['ndvi'] > 0) & (ds['evi'] > 0) & (ds['daymet'] > 0) & (abs(ds['ndvi'] - ds['rolling_median_ndvi']) < ds['rolling_median_ndvi']+ds['rolling_std_ndvi']*2) & (abs(ds['evi'] - ds['rolling_median_evi']) < ds['rolling_median_evi']+ds['rolling_std_evi']*2))
  ds['evi'] = ds['evi'].where((ds['ndvi'] > 0) & (ds['evi'] > 0) & (ds['daymet'] > 0) & (abs(ds['evi'] - ds['rolling_median_evi']) < ds['rolling_median_evi']+ds['rolling_std_evi']*2) & (abs(ds['ndvi'] - ds['rolling_median_ndvi']) < ds['rolling_median_ndvi']+ds['rolling_std_ndvi']*2))
  
  ds['ndvi'] =  ds['ndvi'].interpolate_na(dim='time', method='linear', fill_value="extrapolate")
  ds['evi'] =  ds['evi'].interpolate_na(dim='time', method='linear', fill_value="extrapolate")
  
  ds['ndvi'] = ds['ndvi'].where((ds['ndvi'] > 0) & (ds['evi'] > 0) & (ds['daymet'] > 0) & (abs(ds['ndvi'] - ds['rolling_median_ndvi']) < ds['rolling_median_ndvi']+ds['rolling_std_ndvi']*2) & (abs(ds['evi'] - ds['rolling_median_evi']) < ds['rolling_median_evi']+ds['rolling_std_evi']*2))
  ds['evi'] = ds['evi'].where((ds['ndvi'] > 0) & (ds['evi'] > 0) & (ds['daymet'] > 0) & (abs(ds['evi'] - ds['rolling_median_evi']) < ds['rolling_median_evi']+ds['rolling_std_evi']*2) & (abs(ds['ndvi'] - ds['rolling_median_ndvi']) < ds['rolling_median_ndvi']+ds['rolling_std_ndvi']*2))
  
  ds['ndvi'] =  ds['ndvi'].interpolate_na(dim='time', method='nearest', fill_value="extrapolate")
  ds['evi'] =  ds['evi'].interpolate_na(dim='time', method='nearest', fill_value="extrapolate")
...
def image_average_variables(ds, variable_list, plot=None):
  ...
  if plot!=None:
    sns.lineplot(data=mean_indices, x='time', y='evi', label='evi')
    sns.lineplot(data=mean_indices, x='time', y='ndvi', label='ndvi')
    plt.ylabel('index')
    print('saving plot')
...
with open(path_to_footprint_list) as f:
  ...
  for site in sites:
    ...
    starfm_in_dir='Ameriflux_sites/{}_starfm/starfm_test_no_smooth/'.format(site)
    daymet_in_dir='Ameriflux_sites/{}_starfm/daymet/'.format(site)
    out_dir='Ameriflux_sites/{}_starfm/covariates_smooth_5d/'.format(site)
    out_name='{}_covariates.nc'.format(site)
    out_csv = '{}_indices.csv'.format(site)
    plot_name='{}_index_plot_QA_spatial_rolling.jpg'.format(site)
    ...
    df = image_average_variables(ds, ['evi', 'ndvi', 'daymet', 'qa'], plot=out_dir+plot_name)
