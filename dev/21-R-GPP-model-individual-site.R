...
# Define site for case study
site_name = "sitename"
NEE <- read.table('file-with-NEE-dataset.csv', header=T, sep = ",")
NEE <- NEE[which(NEE$Site==site_name),]
NEE <- NEE[c("Site","Date","SW_IN_NLDAS","SWC1","SWC2","VPD","TS","Clay","STARFM_fPAR","NEE_measured")]
spin <- read.table('file-with-spin-up-dataset.csv', header=T, sep = ",")
spin$Site <- sapply(strsplit(spin$Site, ".csv"),`[`, 1)
spin <- spin[which(spin$Site==site_name),]
spin <- spin[c("Site","DOY","SW_IN_NLDAS","SWC1","SWC2","VPD","TS","Clay","STARFM_fPAR")]
obs = NEE['NEE_measured']
obs <- na.omit(obs)
GPP_data <- read.table('file-with-GPP-dataset.csv', header=T, sep = ",")
GPP_data$GPP_measured <- GPP_data$GPP_measured*24*3600/(10^6)*12
...
C_AGB = C_Pool[which(C_Pool$Site==site_name),c("C_AGB")]
C_BGB = C_Pool[which(C_Pool$Site==site_name),c("C_BGB")]
C_AGL = C_Pool[which(C_Pool$Site==site_name),c("C_AGL")] 
C_BGL = C_Pool[which(C_Pool$Site==site_name),c("C_BGL")]
C_POC = C_Pool[which(C_Pool$Site==site_name),c("C_POC")] 
C_HOC = C_Pool[which(C_Pool$Site==site_name),c("C_HOC")]

# Spinup and NEE model
spin_year = 2002
len = length(unique(NEE$site_num))
...
calcSOC <- function(param, num_sp, tsoil_sp, sm_sp, sm2_sp, clay_sp, GPP_sp, 
  ...
  k_AGL = param[3]
  k_BGL = param[4]
  k_POC = param[5]/10 # Divide back
  k_HOC = param[6]/10 # Divide back
  CSE_AGL = param[7]
  CSE_BGL = param[8]
  CSE_POC = param[9]
  CSE_HOC = param[10]
...
result_model = calcSOC(refPars$best, spin$site_num, spin$TS, spin$SWC1, spin$SWC2, spin$Clay, spin$GPP, 
                       NEE$TS, NEE$SWC1, NEE$SWC2, NEE$Clay, NEE$GPP, NEE$site_num, NEE$NEE_measured) 
...
likelihood <- function(par, sum = TRUE){
  x = refPars$best
  x[parSel] = par
  predicted <- calcSOC(refPars$best, spin$TS, spin$SWC1, spin$SWC2, spin$Clay, spin$GPP, 
                       NEE$TS, NEE$SWC1, NEE$SWC2, NEE$Clay, NEE$GPP, NEE$NEE_measured)[[2]] 
...
# Run Bayesian MCMC calibration and visualize results
out <- runMCMC(bayesianSetup = bayesianSetup, sampler = "DEzs", settings = settings)
post <- data.frame(getSample(out))
post_mean <- colMedians(as.matrix(post))
newresult = calcSOC(post_mean, spin$TS, spin$SWC1, spin$SWC2, spin$Clay, spin$GPP, 
                    NEE$TS, NEE$SWC1, NEE$SWC2, NEE$Clay, NEE$GPP, NEE$NEE_measured)
comparison_before <- data.frame(obs,result_model[[2]],date)
colnames(comparison_before)[3] <- c("Date")
comparison_after <- data.frame(obs,newresult[[2]],date)
colnames(comparison_after)[3] <- c("Date")
lr_before <- lm(NEE_measured ~ NEE_p, data = comparison_before)
r2_before <- summary(lr_before)$r.squared
r2_before 
RMSE_before <- sqrt(mean((comparison_before$NEE_measured - comparison_before$NEE_p)^2))
RMSE_before 
mbe_before  <- mean(comparison_before$NEE_measured - comparison_before$NEE_p)
mbe_before 
lr_after <- lm(NEE_measured ~ NEE_p, data = comparison_after)
r2_after <- summary(lr_after)$r.squared
r2_after 
RMSE_after <- sqrt(mean((comparison_after$NEE_measured - comparison_after$NEE_p)^2))
RMSE_after
mbe_after <- mean(comparison_after$NEE_measured - comparison_after$NEE_p)
mbe_after
output <- data.frame(t(post_mean),r2_before,RMSE_before,mbe_before,r2_after,RMSE_after,mbe_after)
