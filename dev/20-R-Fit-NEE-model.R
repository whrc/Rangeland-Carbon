...
calcSOC <- function(param, num_sp, tsoil_sp, sm_sp, sm2_sp, clay_sp, GPP_sp, 
  ...
  for (k in 1:len){
    ...
    for (n in 1: 1412){
      days = 5 # Change based on how the time-step was set up
      #days = ifelse(n %% 73 != 0, 5, ifelse((n-146) %% 364 == 0, 6, 5))
...
# Calibration setup
parSel = c(1:26) # All parameters here, but changes can be made to only calibrate a set of parameters
likelihood <- function(par, sum = TRUE){
  x = refPars$best
  x[parSel] = par
  predicted <- calcSOC(refPars$best, spin$site_num, spin$TS, spin$SWC1, spin$SWC2, spin$Clay, spin$GPP, 
                       NEE$TS, NEE$SWC1, NEE$SWC2, NEE$Clay, NEE$GPP, NEE$site_num, NEE$NEE_measured) 
...
# Get predicted vs. observed and visualize results
post <- data.frame(getSample(out))
post_mean <- colMedians(as.matrix(post))
newresult = calcSOC(post_mean, spin$site_num, spin$TS, spin$SWC1, spin$SWC2, spin$Clay, spin$GPP, 
                    NEE$TS, NEE$SWC1, NEE$SWC2, NEE$Clay, NEE$GPP, NEE$site_num, NEE$NEE_measured)
comparison_before <- data.frame(obs,result_model,sitelist,date)
colnames(comparison_before)[3:4] <- c("Site","Date")
comparison_after <- data.frame(obs,newresult,sitelist,date)
colnames(comparison_after)[3:4] <- c("Site","Date")
par(mar=c(2.5,2.5,2.5,2.5))
lr_before <- lm(NEE_measured ~ NEE_p, data = comparison_before)
summary(lr_before)
plot(comparison_before$NEE_measured,comparison_before$NEE_p,col=factor(comparison_before$Site),
     main = "Observed vs. predicted NEE before calibration", 
     xlab = "Observed NEE",
     ylab = "Predicted NEE",
     xlim = c(-10,15),
     ylim = c(-10,15))
text(-7.5,7.5, paste0("Model R2= ",round(summary(lr_before)$r.squared,digits = 2)),cex = 1.5,col="red")
legend("topright",legend=levels(factor(comparison_before$Site)),pch = 1,
       col= factor(levels(factor(comparison_before$Site))))
...
summary(lr_after)
plot(comparison_after$NEE_measured,comparison_after$NEE_p,col=factor(comparison_after$Site),
     main = "Observed vs. predicted NEE after calibration", 
     xlab = "Observed NEE",
     ylab = "Predicted NEE",
     xlim = c(-10,10),
     ylim = c(-10,10))
text(-7.5,7.5, paste0("Model R2= ",round(summary(lr_after)$r.squared,digits = 2)),cex = 1.5,col="red")
legend("topright",legend=levels(factor(comparison_after$Site)),pch = 1,
       col= factor(levels(factor(comparison_after$Site))))
...
standard_deviations <- apply(post[, -1], 2, sd)
quartiles <- apply(post[, -1], 2, quantile, probs = c(0.50, 0.05, 0.25, 0.75, 0.95))
output_table <- data.frame(
  `50%` = quartiles[1, ],
  stdDev = standard_deviations,
  `5%` = quartiles[2, ],
  `25%` = quartiles[3, ],
  `75%` = quartiles[4, ],
  `95%` = quartiles[5, ]
