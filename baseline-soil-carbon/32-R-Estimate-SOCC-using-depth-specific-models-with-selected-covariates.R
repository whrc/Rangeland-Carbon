...
CaliAll1 <- read.table(paste0('./SOC/D1/', ROI,'/', ROI,'_calibration_0_Climate_RAP_', KSSL_num,'.csv')
                       , comment.char ="", quote = "\"", header = T, sep = ",")
CaliAll2 <- read.table(paste0('./SOC/D2/', ROI,'/', ROI,'_calibration_0_Climate_RAP_', KSSL_num,'.csv')
                       , comment.char ="", quote = "\"", header = T, sep = ",")
CaliAll3 <- read.table(paste0('./SOC/D3/', ROI,'/', ROI,'_calibration_0_Climate_RAP_', KSSL_num,'.csv')
                       , comment.char ="", quote = "\"", header = T, sep = ",")
...
for (i in 1:length(unique(Cali.i1$Cali_ID1))){
  ...
  rf.i <- randomForest(oc_usda ~ BD + Clay + Sand + SOC + Tree + GPP + EVI +
  ...
  # rf.i <- randomForest(oc_usda ~ ppt + tmin + tmax + VPD + Tree + GPP + EVI + 
  #                       AS + EL + SL + TWI + AFGC + BG + LTR + PFGC + SHR + TREE,
  #                      data = Cali.individual, importance = TRUE, ntree=ntree, mtry=mtry)
...
write.csv(imp.cov,paste0("./SOC/Imp4/D1/",ROI,"/",ROI,"_",KSSL_num,"_",spike_num,"_imp.csv"))
...
rmse.v
mbe.v
write.csv(vali.result, paste0("./SOC/Imp4/D1/",ROI,"/",ROI,"_",KSSL_num,"_",spike_num,"_pred.csv"))
# Medium depth
imp.cov <- data.frame()
preds.i <- data.frame()
Vali.individual.i <- data.frame()
...
colnames(imp.cov) <- names(imp)
imp.cov <- sapply(imp.cov, mean)
write.csv(imp.cov,paste0("./SOC/Imp4/D2/",ROI,"/",ROI,"_",KSSL_num,"_",spike_num,"_imp.csv"))
dotchart(sort(imp.cov), xlab="Variable importance (%)")
colnames(preds.i) <- c("preds.i")
colnames(Vali.individual.i) <- c("Core","Med_depth","vali.i")
vali.result <- cbind(preds.i,Vali.individual.i)
...
rmse.v
mbe.v
write.csv(vali.result, paste0("./SOC/Imp4/D2/",ROI,"/",ROI,"_",KSSL_num,"_",spike_num,"_pred.csv"))
# Bottom depth
imp.cov <- data.frame()
preds.i <- data.frame()
Vali.individual.i <- data.frame()
...
colnames(imp.cov) <- names(imp)
imp.cov <- sapply(imp.cov, mean)
write.csv(imp.cov,paste0("./SOC/Imp4/D3/",ROI,"/",ROI,"_",KSSL_num,"_",spike_num,"_imp.csv"))
dotchart(sort(imp.cov), xlab="Variable importance (%)")
colnames(preds.i) <- c("preds.i")
colnames(Vali.individual.i) <- c("Core","Med_depth","vali.i")
vali.result <- cbind(preds.i,Vali.individual.i)
...
rmse.v
mbe.v
write.csv(vali.result, paste0("./SOC/Imp4/D3/",ROI,"/",ROI,"_",KSSL_num,"_",spike_num,"_pred.csv"))


#######################################################################
# SOC modeling results by combining different depth layers
KSSL_local1 <- read.table(paste0("./SOC/Imp4/D1/",ROI,"/",ROI,"_",KSSL_num,"_",spike_num,"_pred.csv"),
                          comment.char ="", quote = "\"", header = T, sep = ",")
KSSL_local2 <- read.table(paste0("./SOC/Imp4/D2/",ROI,"/",ROI,"_",KSSL_num,"_",spike_num,"_pred.csv"),
                          comment.char ="", quote = "\"", header = T, sep = ",")
KSSL_local3 <- read.table(paste0("./SOC/Imp4/D3/",ROI,"/",ROI,"_",KSSL_num,"_",spike_num,"_pred.csv"),
                          comment.char ="", quote = "\"", header = T, sep = ",")
