...
CaliAll <- read.table(paste0('./SOC/BS/', ROI,'/', ROI,'_calibration_0_', KSSL_num,'.csv')
                      , comment.char ="", quote = "\"", header = T, sep = ",")
...
Cali.i <- Cali.i[,c("sampleids","lay_id","Med_depth","oc_usda","BD","SOC",
                    "Clay","Sand","ppt","tmin","tmax","VPD",
                    "LULC","Tree","GPP","EVI","AS","EL","SL","TWI","Cali_ID")]
Spike <- Spike[,c("Sample_ID","X","Med_depth","SOC_m","BD","SOC",
                  "Clay","Sand","ppt","tmin","tmax","VPD",
...
# Test model performance (pixel-based, KSSL + local) using broad-sclae KSSL dataset
###################################################################################
imp.cov <- data.frame()
preds.i <- data.frame()
...
write.csv(imp.cov,paste0("./SOC/Imp2/",ROI,"/",ROI,"_",KSSL_num,"_",spike_num,"_imp.csv"))
...
rmse.v
mbe.v
write.csv(vali.result, paste0("./SOC/Imp2/",ROI,"/",ROI,"_",KSSL_num,"_",spike_num,".csv"))
