...
if (model_scheme == 0){
  CaliAll <- read.table(paste0('./BD/', ROI,'/', ROI,'_calibration_site_0_', KSSL_num,'.csv')
                        , comment.char ="", quote = "\"", header = T, sep = ",")
} else {
  CaliAll <- read.table(paste0('./BD/', ROI,'/', ROI,'_calibration_0_', KSSL_num,'.csv')
                        , comment.char ="", quote = "\"", header = T, sep = ",")
}
...
Cali.i <- Cali.i[,c("sampleids","lay_id","Med_depth","bd_kssl","BD","SOC","Clay","Sand","ppt","tmin","tmax","VPD",
                      "LULC","Tree","GPP","EVI","AS","EL","SL","TWI","AFGC","BG","LTR","PFGC","SHR","TREE","Cali_ID")]
Spike <- Spike[,c("Sample_ID","X","Med_depth","BD_m","BD","SOC","Clay","Sand","ppt","tmin","tmax","VPD",
...
for (i in 1:length(unique(Cali.i$Cali_ID))){
    ...
    # M1 - without depth
    rf.i <- randomForest(bd_kssl ~ BD + Clay + Sand + SOC + ppt + tmin + tmax + VPD +
                           Tree + GPP + EVI + AS + EL + SL + TWI + AFGC + BG + LTR + PFGC + SHR + TREE,
                         data = Cali.individual, importance = TRUE, ntree=500, mtry=3)
    # M2 - without RAP
    # rf.i <- randomForest(bd_kssl ~ Med_depth + BD + Clay + Sand + SOC + ppt + tmin + tmax + VPD +
    ...
    #                      data = Cali.individual, importance = TRUE, ntree=500, mtry=3)
    # M3 - without climate
    # rf.i <- randomForest(bd_kssl ~ Med_depth + BD + Clay + Sand + SOC +
    #                        Tree + GPP + EVI + AS + EL + SL + TWI + AFGC + BG + LTR + PFGC + SHR + TREE,
    #                      data = Cali.individual, importance = TRUE, ntree=500, mtry=3)
    ...
    #                      data = Cali.individual, importance = TRUE, ntree=500, mtry=3)
    # M6 - without soil
    # rf.i <- randomForest(bd_kssl ~ Med_depth + ppt + tmin + tmax + VPD +
    #                        Tree + GPP + EVI + AS + EL + SL + TWI + AFGC + BG + LTR + PFGC + SHR + TREE,
    #                      data = Cali.individual, importance = TRUE, ntree=500, mtry=3)
    # M7 - depth and soil
    # rf.i <- randomForest(bd_kssl ~ Med_depth + BD + Clay + Sand + SOC,
    ...
    #                      data = Cali.individual, importance = TRUE, ntree=500, mtry=3)
    # M9 - depth and RS
    # rf.i <- randomForest(bd_kssl ~ Med_depth + Tree + GPP + EVI,
    #                      data = Cali.individual, importance = TRUE, ntree=500, mtry=3)
    # M10 - depth and topography
    # rf.i <- randomForest(bd_kssl ~ Med_depth + AS + EL + SL + TWI,
    #                      data = Cali.individual, importance = TRUE, ntree=500, mtry=3)
    # M11 - depth and RAP
    # rf.i <- randomForest(bd_kssl ~ Med_depth + AFGC + BG + LTR + PFGC + SHR + TREE,
    #                      data = Cali.individual, importance = TRUE, ntree=500, mtry=3)
...
write.csv(imp.cov,paste0("./BD/Imp1/",ROI,"/",ROI,"_",KSSL_num,"_",spike_num,"_imp.csv"))
...
rmse.v
mbe.v
write.csv(vali.result, paste0("./BD/Imp1/",ROI,"/",ROI,"_",KSSL_num,"_",spike_num,"_pred.csv"))
...
colnames(imp.cov) <- names(imp)
imp.cov <- sapply(imp.cov, mean)
write.csv(imp.cov,paste0("./BD/Imp1/",ROI,"/",ROI,"_",KSSL_num,"_0_imp.csv"))
dotchart(sort(imp.cov), xlab="Variable importance (%)")
colnames(preds.i) <- c("preds.i")
colnames(Vali.individual.i) <- c("Core","Med_depth","vali.i")
vali.result <- cbind(preds.i,Vali.individual.i)
...
rmse.v
mbe.v
write.csv(vali.result, paste0("./BD/Imp1/",ROI,"/",ROI,"_",KSSL_num,"_0_pred.csv"))


#################################################################################
# Run model with local samples only
# rf <- randomForest(bd_kssl ~ Med_depth + BD + Clay + Sand + SOC + ppt + tmin + tmax + VPD +
#                      Tree + GPP + EVI + AS + EL + SL + TWI + AFGC + BG + LTR + PFGC + SHR + TREE,
#                    data = Spike, importance = TRUE, ntree=500, mtry=3)
# M1 - without depth
rf <- randomForest(bd_kssl ~ BD + Clay + Sand + SOC + ppt + tmin + tmax + VPD +
                       Tree + GPP + EVI + AS + EL + SL + TWI + AFGC + BG + LTR + PFGC + SHR + TREE,
                     data = Spike, importance = TRUE, ntree=500, mtry=3)
# M2 - without RAP
# rf <- randomForest(bd_kssl ~ Med_depth + BD + Clay + Sand + SOC + ppt + tmin + tmax + VPD +
...
#                      data = Spike, importance = TRUE, ntree=500, mtry=3)
# M3 - without climate
# rf <- randomForest(bd_kssl ~ Med_depth + BD + Clay + Sand + SOC +
#                        Tree + GPP + EVI + AS + EL + SL + TWI + AFGC + BG + LTR + PFGC + SHR + TREE,
#                      data = Spike, importance = TRUE, ntree=500, mtry=3)
# M4 - without topography
# rf <- randomForest(bd_kssl ~ Med_depth + BD + Clay + Sand + SOC + ppt + tmin + tmax + VPD +
...
#                      data = Spike, importance = TRUE, ntree=500, mtry=3)
# M6 - without soil
# rf <- randomForest(bd_kssl ~ Med_depth + ppt + tmin + tmax + VPD +
#                        Tree + GPP + EVI + AS + EL + SL + TWI + AFGC + BG + LTR + PFGC + SHR + TREE,
#                      data = Spike, importance = TRUE, ntree=500, mtry=3)
# M7 - depth and soil
# rf <- randomForest(bd_kssl ~ Med_depth + BD + Clay + Sand + SOC,
...
#                      data = Spike, importance = TRUE, ntree=500, mtry=3)
# M9 - depth and RS
# rf <- randomForest(bd_kssl ~ Med_depth + Tree + GPP + EVI,
#                      data = Spike, importance = TRUE, ntree=500, mtry=3)
# M10 - depth and topography
# rf <- randomForest(bd_kssl ~ Med_depth + AS + EL + SL + TWI,
#                      data = Spike, importance = TRUE, ntree=500, mtry=3)
# M11 - depth and RAP
# rf <- randomForest(bd_kssl ~ Med_depth + AFGC + BG + LTR + PFGC + SHR + TREE,
#                      data = Spike, importance = TRUE, ntree=500, mtry=3)
r2.c <- summary(lm(rf$predicted ~ Spike$bd_kssl))$adj.r.squared
r2.c
resids.c <- rf$predicted - Spike$bd_kssl
rmse.c <- sqrt(mean(resids.c^2))
rmse.c
mbe.c <- mean(rf$predicted - Spike$bd_kssl)
mbe.c
imp <- importance(rf, type = 1)[,1]
write.csv(imp, paste0("./BD/Imp1/",ROI,"/",ROI,"_0_",spike_num,"_imp.csv"))
dotchart(sort(imp), xlab="Variable importance (%)")
preds <- predict(rf, newdata = Vali)
r2.v <- summary(lm(preds ~ Vali$BD_m))$adj.r.squared
r2.v
resids.v <- preds - Vali$BD_m
rmse.v <- sqrt(mean(resids.v^2))
rmse.v
mbe.v <- mean(preds - Vali$BD_m)
mbe.v
plot(Vali$BD_m, preds, 
     main = "Measured vs. predicted BD of the validation set \n
     ...
     ylim = c(0,2))
abline(coef = c(0,1))
vali.result <- cbind(Vali[,c("Num","Med_depth","BD_m")],preds)
colnames(vali.result) <- c("Core","Med_depth","vali.i","preds.i")
write.csv(vali.result, paste0("./BD/Imp1/",ROI,"/",ROI,"_0_",spike_num,"_pred.csv"))
