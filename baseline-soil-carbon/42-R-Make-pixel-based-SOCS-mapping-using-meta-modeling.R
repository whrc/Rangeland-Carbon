...
#Get every 10 data points for meta-modeling
imagedata <- imagedata[which((imagedata$pred.ID %% 10 == 0)),]
pred.ID <- pred.ID[which((pred.ID$ID %% 10 == 0)),]
...
write.csv(imp.cov,paste0("./",ROI,"_",KSSL_num,"_pred_SOCS_imp_meta.csv"))
...
write.csv(preds.i.reorg,paste0("./",ROI,"_",KSSL_num,"_pred_SOCS_meta.csv"))


###############################################################
# Run meta-model and create meta-model based maps
# Build meta-model
finalmodel_SOCS <- randomForest(Pred_SOCS ~ BD + Clay + Sand + SOC + 
                                  ppt + tmin + tmax + VPD + 
                                  Tree + GPP + EVI + 
                                  AS + EL + SL + TWI +
                                  AFGC + BG + LTR + PFGC + SHR + TREE,
                                data = preds.i.reorg, importance = TRUE, ntree=500, mtry=3)
r2_SOCS <- summary(lm(finalmodel_SOCS$predicted ~ SOCS_pred$Pred_SOCS))$adj.r.squared
r2_SOCS
resids_SOCS <- finalmodel_SOCS$predicted - SOCS_pred$Pred_SOCS
mbe_SOCS <- mean(resids_SOCS)
mbe_SOCS
rmse_SOCS <- sqrt(mean(resids_SOCS^2))
rmse_SOCS
impToPlot_SOCS <- importance(finalmodel_SOCS, type = 1)
preds.mm.SOCS <- predict(finalmodel_SOCS, newdata = imagedata)
meta.result.SOCS <- cbind(imagedata,preds.mm.SOCS)
meta.result.SOCS <- cbind(meta.result.SOCS[,1:3], meta.result.SOCS[,dim(meta.result.SOCS)[2]])
colnames(meta.result.SOCS)[4] <- c("Pred_SOCS")
write.csv(meta.result.SOCS,paste0("./",ROI,"_",KSSL_num,"_pred_SOCS_meta_modeling.csv"))
# Create maps
...
SOCS_map <- rasterize(meta.result.SOCS[, c('x', 'y')], x,
                      meta.result.SOCS[, 'Pred_SOCS'], fun=mean, na.rm = TRUE)
plot(SOCS_map)
filepath <- paste0("./",ROI,"_",KSSL_num,"_predicted_SOCS_meta_modeling_map")
