...
# Extract SOC density calibration set at the pixel level
###############################################################
# Import calibration (KSSL) and validation (Year 1 samples) datasets
Cali<- read.table('./KSSL.f.SOCSd.csv', comment.char ="", quote = "\"", header = T, sep = ",")
Vali<- read.table('./CSV-with-SOC-density-and-site-covariates-filename.csv',comment.char ="", quote = "\"", header = T, sep = ",")
Cali$Dataset <- "Cali"
Vali$Dataset <- "Vali"
# Refine KSSL dataset
...
Vali_reorg <- Vali[,c("Dataset","X","Sample_ID","Num","Med_depth","Measured_SOC","Measured_BD","density","BD","SOC","Clay","Sand","ppt","tmin","tmax","VPD",
...
Cali_reorg <- Cali_reorg[,c("Dataset","X.1","sampleids","lay_id","Med_depth","oc_usda","bd_kssl","density",
...
combo.p <- combo[,c(9:16,18:30)]# Exclude LULC (categorical) covariate in the model
...
write.csv(calibration.i,paste0("./SOCD/",ROI,"/",ROI,"_calibration_0_",KSSL_num,".csv"))


# Extract SOC density calibration set at the site-level
###############################################################
# Import calibration (KSSL) and validation (Year 1 samples) datasets
Cali<- read.table('./KSSL.f.SOCSd.csv', comment.char ="", quote = "\"", header = T, sep = ",")
Vali<- read.table('./CSV-with-SOC-density-and-site-covariates-filename.csv',comment.char ="", quote = "\"", header = T, sep = ",")
...
write.csv(calibration.site,paste0("./SOCD/",ROI,"/",ROI,"_calibration_site_0_",KSSL_num,".csv"))