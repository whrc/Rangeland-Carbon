...
combo.p <- combo[,c(11:14,23:28)] # Use only climate and RAP covariates
...
write.csv(calibration.i,paste0("./SOC/D1/",ROI,"/",ROI,"_calibration_0_Climate_RAP_",KSSL_num,".csv"))
...
calibration.i <- cbind(calibration.i[,4],calibration.i[,3])
colnames(calibration.i)[1:2] <- c("predid","sampleids")
write.csv(calibration.i,paste0("./SOC/D2/",ROI,"/",ROI,"_calibration_0_Climate_RAP_",KSSL_num,".csv"))
...
calibration.i <- cbind(calibration.i[,4],calibration.i[,3])
colnames(calibration.i)[1:2] <- c("predid","sampleids")
write.csv(calibration.i,paste0("./SOC/D3/",ROI,"/",ROI,"_calibration_0_Climate_RAP_",KSSL_num,".csv"))
...
combo <- rbind(Cali_reorg, Vali_reorg) # Combine calibration and prediction/validation sets
combo.p <- combo[,c(12:15,24:29)] # Use only climate and RAP covariates
combo.p.pca = prcomp(combo.p, scale. = TRUE) # Use scaled values to run PCA
var_explained <- combo.p.pca$sdev^2/sum(combo.p.pca$sdev^2)
scores.p = combo.p.pca$x
s.p <- scores.p [,1:(max(which(var_explained > 0.05)))] # Retain PCs explained >5% 
...
calibration.i <- cbind(calibration.i[,4],calibration.i[,3])
colnames(calibration.i)[1:2] <- c("predid","sampleids")
write.csv(calibration.i,paste0("./BD/D1/",ROI,"/",ROI,"_calibration_0_Climate_RAP_",KSSL_num,".csv"))
# For the medium layer
Cali <- Cali0[which((Cali0$Med_depth >30)&(Cali0$Med_depth < 60)),]
Vali <- Vali0[which((Vali0$Med_depth >30)),]
# First reorganize the datasets in order to combine them for PCA analysis
...
calibration.i <- cbind(calibration.i[,4],calibration.i[,3])
colnames(calibration.i)[1:2] <- c("predid","sampleids")
write.csv(calibration.i,paste0("./BD/D2/",ROI,"/",ROI,"_calibration_0_Climate_RAP_",KSSL_num,".csv"))
# For the bottom layer
Cali <- Cali0[which((Cali0$Med_depth >30)&(Cali0$Med_depth < 60)),]
Vali <- Vali0[which((Vali0$Med_depth >30)),]
# First reorganize the datasets in order to combine them for PCA analysis
...
calibration.i <- cbind(calibration.i[,4],calibration.i[,3])
colnames(calibration.i)[1:2] <- c("predid","sampleids")
write.csv(calibration.i,paste0("./BD/D3/",ROI,"/",ROI,"_calibration_0_Climate_RAP_",KSSL_num,".csv"))