...
CaliAll1 <- read.table(paste0('./BD/D1/', ROI,'/', ROI,'_calibration_0_Climate_RAP_', KSSL_num,'.csv')
                       , comment.char ="", quote = "\"", header = T, sep = ",")
CaliAll2 <- read.table(paste0('./BD/D2/', ROI,'/', ROI,'_calibration_0_Climate_RAP_', KSSL_num,'.csv')
                       , comment.char ="", quote = "\"", header = T, sep = ",")
CaliAll3 <- read.table(paste0('./BD/D3/', ROI,'/', ROI,'_calibration_0_Climate_RAP_', KSSL_num,'.csv')
                       , comment.char ="", quote = "\"", header = T, sep = ",")
...
Cali.i1 <- Cali.i1[,c("sampleids","lay_id","Med_depth","bd_kssl","BD","SOC","Clay","Sand","ppt","tmin","tmax","VPD",
                      "LULC","Tree","GPP","EVI","AS","EL","SL","TWI","AFGC","BG","LTR","PFGC","SHR","TREE","Cali_ID1")]
Cali.i2 <- Cali.i2[,c("sampleids","lay_id","Med_depth","bd_kssl","BD","SOC","Clay","Sand","ppt","tmin","tmax","VPD",
                      "LULC","Tree","GPP","EVI","AS","EL","SL","TWI","AFGC","BG","LTR","PFGC","SHR","TREE","Cali_ID2")]
Cali.i3 <- Cali.i3[,c("sampleids","lay_id","Med_depth","bd_kssl","BD","SOC","Clay","Sand","ppt","tmin","tmax","VPD",
...
for (i in 1:length(unique(Cali.i1$Cali_ID1))){
  ...
  # Test the use of covariates by excluding climate factors
  rf.i <- randomForest(bd_kssl ~ BD + Clay + Sand + SOC + Tree + GPP + EVI +
                         AS + EL + SL + TWI + AFGC + BG + LTR + PFGC + SHR + TREE,
                       data = Cali.individual, importance = TRUE, ntree=ntree, mtry=mtry)
  # Test the use of climate, RS, and topographic covariates 
  # rf.i <- randomForest(bd_kssl ~ ppt + tmin + tmax + VPD + Tree + GPP + EVI + 
  #                       AS + EL + SL + TWI + AFGC + BG + LTR + PFGC + SHR + TREE,
  #                      data = Cali.individual, importance = TRUE, ntree=ntree, mtry=mtry)
...
write.csv(imp.cov,paste0("./BD/Imp4/D1/",ROI,"/",ROI,"_",KSSL_num,"_",spike_num,"_imp.csv"))
...
rmse.v
mbe.v
write.csv(vali.result, paste0("./BD/Imp4/D1/",ROI,"/",ROI,"_",KSSL_num,"_",spike_num,"_pred.csv"))
# Medium depth
imp.cov <- data.frame()
preds.i <- data.frame()
Vali.individual.i <- data.frame()
...
colnames(imp.cov) <- names(imp)
imp.cov <- sapply(imp.cov, mean)
write.csv(imp.cov,paste0("./BD/Imp4/D2/",ROI,"/",ROI,"_",KSSL_num,"_",spike_num,"_imp.csv"))
dotchart(sort(imp.cov), xlab="Variable importance (%)")
colnames(preds.i) <- c("preds.i")
colnames(Vali.individual.i) <- c("Core","Med_depth","vali.i")
vali.result <- cbind(preds.i,Vali.individual.i)
...
rmse.v
mbe.v
write.csv(vali.result, paste0("./BD/Imp4/D2/",ROI,"/",ROI,"_",KSSL_num,"_",spike_num,"_pred.csv"))
# Bottom depth
imp.cov <- data.frame()
preds.i <- data.frame()
Vali.individual.i <- data.frame()
...
colnames(imp.cov) <- names(imp)
imp.cov <- sapply(imp.cov, mean)
write.csv(imp.cov,paste0("./BD/Imp4/D3/",ROI,"/",ROI,"_",KSSL_num,"_",spike_num,"_imp.csv"))
dotchart(sort(imp.cov), xlab="Variable importance (%)")
colnames(preds.i) <- c("preds.i")
colnames(Vali.individual.i) <- c("Core","Med_depth","vali.i")
vali.result <- cbind(preds.i,Vali.individual.i)
...
rmse.v
mbe.v
write.csv(vali.result, paste0("./BD/Imp4/D3/",ROI,"/",ROI,"_",KSSL_num,"_",spike_num,"_pred.csv"))


#######################################################################
# BD modeling results by combining different depth layers
KSSL_local1 <- read.table(paste0("./BD/Imp4/D1/",ROI,"/",ROI,"_",KSSL_num,"_",spike_num,"_pred.csv"),
                          comment.char ="", quote = "\"", header = T, sep = ",")
KSSL_local2 <- read.table(paste0("./BD/Imp4/D2/",ROI,"/",ROI,"_",KSSL_num,"_",spike_num,"_pred.csv"),
                          comment.char ="", quote = "\"", header = T, sep = ",")
KSSL_local3 <- read.table(paste0("./BD/Imp4/D3/",ROI,"/",ROI,"_",KSSL_num,"_",spike_num,"_pred.csv"),
                          comment.char ="", quote = "\"", header = T, sep = ",")
