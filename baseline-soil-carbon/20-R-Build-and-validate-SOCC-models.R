...
if (model_scheme == 0){
  CaliAll <- read.table(paste0('./SOC/', ROI,'/', ROI,'_calibration_site_0_', KSSL_num,'.csv')
                        , comment.char ="", quote = "\"", header = T, sep = ",")
} else {
...
if (model_scheme == 1) { # Pixel-based model
  ...
  write.csv(imp.cov,paste0("./SOC/",ROI,"/",ROI,"_",scheme,"_",KSSL_num,"_",spike_num,"_imp.csv"))
  ...
  write.csv(vali.result, paste0("./SOC/",ROI,"/",ROI,"_",scheme,"_",KSSL_num,"_",spike_num,"_pred.csv"))
}
if (model_scheme == 0) { # Site-based model
  Spike <- Spike[,c("Sample_ID","Med_depth","SOC_m","BD","SOC","Clay","Sand","ppt","tmin","tmax","VPD",
                    "LULC","Tree","GPP","EVI","AS","EL","SL","TWI","AFGC","BG","LTR","PFGC","SHR","TREE")]
  Cali <- Cali[,c("sampleids","Med_depth","oc_usda","BD","SOC","Clay","Sand","ppt","tmin","tmax","VPD",
                  "LULC","Tree","GPP","EVI","AS","EL","SL","TWI","AFGC","BG","LTR","PFGC","SHR","TREE")]
  ...
  r2.c <- summary(lm(rf$predicted ~ Site_Cali$oc_usda))$adj.r.squared
  r2.c
  resids.c <- rf$predicted - Site_Cali$oc_usda
  rmse.c <- sqrt(mean(resids.c^2))
  rmse.c
  mbe.c <- mean(rf$predicted - Site_Cali$oc_usda)
  mbe.c
  imp <- importance(rf, type = 1)[,1]
  write.csv(imp,paste0("./SOC/",ROI,"/",ROI,"_",scheme,"_",KSSL_num,"_",spike_num,"_imp.csv"))
...
if (model_scheme == 1) { # Pixel-based model
  ...
  colnames(imp.cov) <- names(imp)
  imp.cov <- sapply(imp.cov, mean)
  write.csv(imp.cov,paste0("./SOC/",ROI,"/",ROI,"_",scheme,"_",KSSL_num,"_0_imp.csv"))
  dotchart(sort(imp.cov), xlab="Variable importance (%)")
  colnames(preds.i) <- c("preds.i")
  colnames(Vali.individual.i) <- c("Core","Med_depth","vali.i")
  vali.result <- cbind(preds.i,Vali.individual.i)
  ...
  abline(coef = c(0,1))
  vali.result <- vali.result[,c("Core","Med_depth","vali.i","preds.i")]
  write.csv(vali.result, paste0("./SOC/",ROI,"/",ROI,"_",scheme,"_",KSSL_num,"_0_pred.csv"))
}
if (model_scheme == 0) { # Site-based model
  rf <- randomForest(oc_usda ~ Med_depth + BD + Clay + Sand + SOC + ppt + tmin + tmax + VPD +
                       Tree + GPP + EVI + AS + EL + SL + TWI + AFGC + BG + LTR + PFGC + SHR + TREE,
                     data = Cali, importance = TRUE, ntree=500, mtry=3)
  r2.c <- summary(lm(rf$predicted ~ Cali$oc_usda))$adj.r.squared
  r2.c
  resids.c <- rf$predicted - Cali$oc_usda
  rmse.c <- sqrt(mean(resids.c^2))
  rmse.c
  mbe.c <- mean(rf$predicted - Cali$oc_usda)
  mbe.c
  imp <- importance(rf, type = 1)[,1]
  write.csv(imp,paste0("./SOC/",ROI,"/",ROI,"_",scheme,"_",KSSL_num,"_0_imp.csv"))
  dotchart(sort(imp), xlab="Variable importance (%)")
  preds <- predict(rf, newdata = Vali)
  r2.v <- summary(lm(preds ~ Vali$Measured_SOC))$adj.r.squared
  resids.v <- preds - Vali$Measured_SOC
...
mbe.c
imp <- importance(rf, type = 1)[,1]
write.csv(imp, paste0("./SOC/",ROI,"/",ROI,"_0_",spike_num,"_imp.csv"))
dotchart(sort(imp), xlab="Variable importance (%)")
preds <- predict(rf, newdata = Vali)
r2.v <- summary(lm(preds ~ Vali$Measured_SOC))$adj.r.squared
r2.v
...
vali.result <- cbind(Vali[,c("Num","Med_depth","Measured_SOC")],preds)
colnames(vali.result) <- c("Core","Med_depth","vali.i","preds.i")
write.csv(vali.result, paste0("./SOC/",ROI,"/",ROI,"_0_",spike_num,"_pred.csv"))